...
function AddQuestion() {
  ...
  useEffect(() => {
    const fetchQuizCodes = async () => {
      try {
        const res = await axios.get('https://quiz-master-backend-3.onrender.com/quiz/quizzes');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://quiz-master-backend-3.onrender.com/quiz/add-question', {
        text,
